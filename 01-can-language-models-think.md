...
If we accept that humans at large do think, we would like to relax this test somewhat to include them. We will continue to assume that we are comparing only trials of the Turing test where the interrogator is the most skilled, such that in many tests between many interrogators and many human respondents, we measure only how successful the best interrogators are at discerning humans from programs. We would require of the human respondents only that they be drawn from some reasonably sized population more or less at random, and that they sincerely be attempting to convince the interrogator that they are human during each trial. We would have to concede that a program was thinking as well as a human if its percentile was at least 50% considering the best-performing interrogators. If the program had any notable shortcoming relative to most humans, the interrogators could exploit that and its percentile would be far below 50%.

## 4. Old Objections

We will take for granted that there is some program which could possibly be constructed which would pass such a test. Because our working definition of thinking is "able to do all of the sorts of things a human can do", this program thinks. We can be certain that whatever it is doing internally is at least as complex as what a human being does when writing or speaking. Turing's paper on the test itself helpfully addresses most of the plausible-seeming objections to the notion that some program could, in principle, think.
...
This passage has aged very nicely. It is especially amusing to see "make mistakes" on the list. Most of the time when we see the Arguments from Various Disabilities made now, they concern some current category of common mistakes from LLMs.

Let us summarize the most common such disabilities regarding current language models. They are bad at arithmetic, they rhyme poorly, they cannot count letters well, they are prone to odd spelling errors, they repeat themselves (especially after mistakes), they have no long term memory, they are often poor conversationalists, and they make things up constantly.

These are all very convincing reasons not to use these language models without human review to write legal motions, tell you if an essay is plagiarized, or diagnose potentially life-threatening medical conditions. No language model that currently exists deserves an M.D, a J.D., or a chair on a graduation committee. Whether we trust a specific program for a specific task currently is a very specific question, and should be considered separately from determining whether similar programs are or could be capable of doing those sorts of tasks.

Turing's remarks state, it seems correctly, that all assertions about machines with specific disabilities being unable to think are made out of induction, that is, because people have never seen a program that could do such a thing, they think that no program ever could. This reasoning is categorically incorrect. It was incorrect in 1950 when Turing was stating that it was used to argue that no program could use words properly, and it is incorrect now.
...
X is variously: Math, matrices\*, statistics, electricity, training data. There are surely many more.
...
We will confine ourselves primarily to the types of programs we call neural networks*. We will try to explain them entirely mathematically, without reference to biological neurons. A neural network is a program which takes some numbers as input, passes it through "layers" which each perform some specific mathematical operation on those numbers, and then outputs the resulting numbers.

An architecture specifies the number, type, and order of the layers of the neural network. Often the number of layers is flexible, and a given operation can be repeated or repeated with modifications many times. Architectures will determine some set of operations which are static, and some which can change or are "trainable". An architecture determines what each layer takes as input, and whether its output goes to other layers or becomes the program output.

In some limited cases it seems true that a given architecture cannot possibly think. A single layer neural network cannot compute the XOR\* function, for one famous example. The XOR function can successfully implemented by two switches attached to one lightbulb. Certainly, human thought is at least as complex as XOR, so anything which cannot compute XOR cannot think. This is a good argument against a single-layer neural network being able to think. This was remedied by constructing neural networks with more than one layer.

Generally speaking no architecture in serious use in 2024 has any obvious limitations of this kind. They are Turing-complete\*, or as close to Turing-complete as anything which has finite memory can be. (For any unfamiliar: This means that it can perform any possible computation.) There are theoretical arguments about how complex a calculation some modern architectures can perform, but these tend to be tasks requiring very large working memories which humans would also struggle with.

It is very difficult to be sure, given that current architectures are Turing-complete and that they are often modified in many ways, that they categorically cannot do any particular thing. It is plausible that no current version of a neural network can do some specific thing efficiently enough to make doing that thing feasible on any hardware we can make. However, architectures are varied upon constantly. Our efficiency at completing the same task has increased many times over in recent years. To state that a given architecture, exactly as it currently exists, cannot do any particular thing efficiently is not an argument not to use that architecture at all. It is an argument to modify it, as we can and have done many times.

As we generalize the approach: Either some program can think, or no program can. If some program can think, some architecture can, since they are Turing-complete. If some architecture can think, any specific disability you can find in a current architecture will give you a hint about what must be true of an architecture that could think.

Generally speaking, the principle of "stacking an arbitrary number of layers in some clever way" has consistently proven that it solves any problem we can find a way to use it on. It is a general method of solving problems for which a solution is difficult to write out precisely, the same way writing is a general method of conveying meaning and computer programs written in programming languages are general purpose methods of doing well-defined math. Arguing against a specific architecture is either arguing in favor of finding an alternative architecture with specific advantages or failing to consider that alternatives could exist.
...
It is fair to object that games are by their nature small, and perhaps what we consider "original" is too broad to be captured by them. This is a fair objection. It is worth noting: This was not commonly said before programs became much better than humans at these games. Once upon a time, it was not uncommon to hear it said that only a truly sentient being could play chess well. We know now that this is not true.

Lady Lovelace also states that a program cannot anticipate analytic relations or truths (ie, do mathematical proofs). As it happens programs often do this now, although there is no way Ada could not have known that this would happen. At most we can say that programs are only pretty good, and not excellent, at doing mathematical proofs now. I do not fear that it will age poorly if I say that people now living will see a world where mathematical proofs, like chess, are something that everyone just accepts programs are better at than humans.
...
A certain researcher stated that his personal test was whether or not language models understood his jokes, and now they do. (He seems somewhat worried, therefore, that AI might kill us all. We will save this question for a later document.) (Knight, 2023)[<sup>10</sup>](#citation-10) Anyone who had such a litmus test, which boiled down to the model understanding English, has probably seen it passed recently by contemporary models. If they haven't, they will soon.

We seem to have very quickly forgotten this, but once upon a time it would have been common to say that this would mean that we had solved AI as a whole, and that such a program would surely be as smart as a human in every way. Much like chess, this has turned out to not be true. Language models understand language, but they have various serious disabilities and in some ways are clearly not as capable as humans.
...
Thought exists on a spectrum. You can often see that humans who cannot use language at all are still thinking. Because we are human too, their actions often show us their thoughts. We can see their thoughts on their faces, in their eyes, in their movements. We experience their thoughts almost as if they are our own. Generally speaking, we can be certain that other humans do think even if they are not very good at convincing you of who they are in writing, or with language at all.

I would argue that current language models do think to some degree, but they do not think as humans do. Neither the development of any specific language model nor the line of research as a whole follows the path of human development. They do not follow human history for any specific human, nor for humanity as a whole. We cannot expect them to parallel humans in what they can and cannot do. They do not parallel humans who have specific disabilities, either. Although they can think in the sense of "use language intelligibly", they do not have coherent personalities or motivations and they do not really seem like they are beings to us. They are computer programs that have had more or less the entire internet compressed into them. This results in something that seems more like a bit of subconscious or stray brain matter than a human being.
...
Our other possibility is that "strong originality" requires extrapolating successfully. It can be argued, we think persuasively, that current models in the year 2024 fail to do this very much currently. It is possible that when we seek "true" originality, originality that feels human, we need the model to be able to consistently explore outside of any path that any human has already carved in anything yet written. It also seems likely that a research paradigm that achieved this would achieve *much more* than just making a model seem, finally, "creative" when it wrote essays.
